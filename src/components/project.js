...
const ProjectPage = () => {
    return (
        <div className="container max-w-6xl mx-auto px-4 mt-25 -mb-20 sm:px-12 md:px-20 py-10 flex flex-col items-center justify-center"> 
            <Fade right>
                <h1 className="font-mono" style={{color: 'var(--primary)'}}>{'<'} projects {' />'}</h1>
            </Fade>
            <ul className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                <Fade bottom>
                    {projects.map((project) => {
                        return (
                            <li className="flex flex-col shadow-md rounded-2xl p-4 hover:shadow-xl transform hover:scale-105 transition duration-500 hover:content highlight" style={{background: 'var(--card)'}}>
